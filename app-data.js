...
const HANI_REACTIONS={
  letter:{badge:'반짝 이름표가 빛나요',text:'좋아요! 이름표가 반짝였어요. 편지가 자기 이름을 조금 기억해 냈어요.',log:'글자 숲에서 첫 빛 조각을 찾았어요.'},
  word:{badge:'노란 꽃씨 주머니가 흔들려요',text:'단어 꽃이 활짝 피었어요. 하얀 편지에 따뜻한 색이 돌아오고 있어요.',log:'단어 동산에서 두 번째 빛 조각을 모았어요.'},
  play:{badge:'맑은 소리 종이 울려요',text:'소리 종이 맑게 울렸어요. 소리 동굴 문이 열리고 마을 주문이 깨어났어요!',log:'소리 동굴에서 세 번째 빛 조각을 찾았어요.'},
  find:{badge:'돋보기가 반짝반짝',text:'여러 글자 속에서 오늘 글자를 눈으로 콕 찾았어요! 하니 눈이 반짝반짝, 정말 대단해요!',log:'글자 찾기에서 오늘 글자를 눈으로 찾아냈어요.'},
  all:{badge:'별빛 우체국이 환해졌어요',text:'오늘의 편지가 다시 읽히기 시작했어요. 하니와 함께 만든 이야기예요!',log:'오늘의 모험을 모두 완료했어요. 새 편지가 내일을 기다려요.'}
};
...
const CURRICULUM=[
  ...
  {act:8,key:'sentence',type:'sentence',title:'8막 이야기 책',place:'별빛 우체국',
   ...
   // 마지막 화(피날레): 오프닝에서 하얗게 지워졌던 그 편지. 아이가 스스로 읽으면 여정이 닫힌다.
   finale:{sent:'안녕 하니 사랑해 또 보자',
           words:['안녕','하니','사랑해','또','보자'],
           cues:['👋','🐥','💛','✨','🤝']},
...
const EPISODE_PATH=(function(){var p=[];CURRICULUM.forEach(function(a){
  if(a.type==='letter'){var isFin=(a.key==='final');a.letters.forEach(function(ch){var node={act:a.act,actKey:a.key,actTitle:a.title,place:a.place,type:'letter',ch:ch};if(isFin)node.final=true;p.push(node);});
    ...
    if(a.finale)p.push({act:a.act,actKey:a.key,actTitle:a.title,place:a.place,type:'finale',
      sent:a.finale.sent,words:a.finale.words.slice(),cues:a.finale.cues.slice()});}
...
// 막 단위 이야기 줄기 — 이번 막에서 무엇을 되찾고, 다음 막은 어디인지.
// (기존 STORY_ARC/STORY_CHAPTERS는 하루 3활동 루프 전용이라 막 구조를 못 덮었다)
const ACT_ARC={
  1:{gain:'숨어 버린 모음의 빛을 되찾아요.',next:'다음은 자음 친구들이 기다리는 글자 숲이에요.'},
  2:{gain:'숲에 숨은 자음 친구들을 깨워요.',next:'다음은 글자를 만드는 대장간, 글자 공방이에요.'},
  3:{gain:'자음과 모음을 두드려 글자를 만들어요.',next:'다음은 무거운 돌문, 받침의 문이에요.'},
  4:{gain:'글자 아래 받침돌을 얹어 끝소리를 되찾아요.',next:'다음은 메아리가 사는 소리 동굴이에요.'},
  5:{gain:'힘껏 내는 쌍둥이 소리를 되찾아요.',next:'다음은 별빛 뒤에 숨은 모음들이에요.'},
  6:{gain:'겹쳐서 숨어 있던 모음들을 찾아내요.',next:'다음은 꽃이 시든 단어 마을이에요.'},
  7:{gain:'글자를 모아 단어를 지어 마을을 살려요.',next:'다음은 마지막, 별빛 우체국이에요.'},
  8:{gain:'문장을 읽어 하얗게 비었던 편지를 되살려요.',next:'마지막에는 그 편지를 스스로 읽어요.'},
};

// 앱이 소리내어 읽는 UI 문구 — 데이터로 모아 두면 음성 생성 스크립트(generate_voices.py)와
// 검사기(tools/check_content.py)가 같은 목록을 보게 되어 "화면엔 있는데 MP3가 없는" 문구가
// 생기지 않는다. 이전에는 막 클리어·졸업 같은 가장 중요한 순간이 전부 기기 TTS로 떨어졌다.
// 받침 유무에 따라 목적격 조사를 고른다('열쇠를' / '돌을'). 음성으로 읽히므로 틀리면 어색하다.
function josaEul(word){
  var last=String(word||'').slice(-1);
  var code=last.charCodeAt(0)-0xAC00;
  if(code<0||code>=11172)return '을';       // 한글 음절이 아니면 안전하게 '을'
  return (code%28)===0?'를':'을';           // 종성 없음 → 를
}
const SPOKEN_UI={
  actClear:function(act){return act+'막 클리어';},
  relicGot:function(name){return name+josaEul(name)+' 되찾았어요';},
  praise:'정말 잘했어요',
  read:'읽었어요',
  batchim:'받침',
  finaleRead:'편지를 스스로 읽었어요',
  finaleAlive:'별빛 편지가 살아났어요',
  finaleDeliver:'하니가 편지를 배달할게요',
};
// 위 문구가 실제로 만들어 낼 수 있는 텍스트 전부(음성 생성·커버리지 검사용).
const SPOKEN_UI_TEXTS=(function(){
  var out=[SPOKEN_UI.praise,SPOKEN_UI.read,SPOKEN_UI.batchim,
           SPOKEN_UI.finaleRead,SPOKEN_UI.finaleAlive,SPOKEN_UI.finaleDeliver];
  for(var a=1;a<=8;a++)out.push(SPOKEN_UI.actClear(a));
  CURRICULUM.forEach(function(c){if(c.relic)out.push(SPOKEN_UI.relicGot(c.relic));});
  return out;
})();

// 막마다 함께한 도우미 — 막을 클리어할 때 작별 인사로 다시 등장한다.
// 이전에는 막 시작 그림책 2쪽에서 한 번 소개되고 여정 어디에도 다시 나오지 않아,
// 아이와 관계가 쌓이지 않았다. art는 위에 정의된 손그림 SVG 빌더 함수 자체.
const ACT_HELPERS={
  1:{name:'반딧불이 깜빡이',art:aiFirefly,bye:'깜빡이가 꼬리불을 반짝! "모음의 빛을 되찾아 줘서 고마워요."'},
  2:{name:'숲지기 도토리',art:aiSquirrel,bye:'도토리가 꼬리를 살랑! "자음 친구들이 숲으로 돌아왔어요. 고마워요!"'},
  3:{name:'대장장이 곰 뚝딱',art:aiBear,bye:'뚝딱이가 망치를 내려놓아요. "이제 글자를 만들 줄 아는군요. 멋져요!"'},
  4:{name:'두꺼비 끄떡',art:aiToad,bye:'끄떡이가 크게 끄덕! "받침돌을 다 얹었어요. 문이 활짝 열렸어요."'},
  5:{name:'쌍둥이 박쥐',art:aiBats,bye:'쌍둥이 박쥐가 함께 날개짓! "센 소리를 되찾아 줘서 고마워요."'},
  6:{name:'별빛 요정',art:aiFairy,bye:'요정이 지팡이를 반짝! "숨어 있던 모음들이 모두 나왔어요."'},
  7:{name:'나비 팔랑이',art:aiButterfly,bye:'팔랑이가 팔랑팔랑! "단어 꽃이 마을 가득 피었어요. 고마워요!"'},
  8:{name:'하니',art:null,bye:'하니가 편지를 품에 안아요. "이제 온 마을에 배달하러 갈게요!"'},
};
...
const ACT_INTROS={
  ...
  3:{act:3,pages:[
    ...
    {cap:'직접 만들어요',say:'ㄱ 더하기 ㅏ는 가! 이번엔 우리가 망치를 들고 글자를 만들어요. 편지에서 사라진 글자를 되찾아 볼까요?',
  ...
  7:{act:7,pages:[
    ...
    {cap:'단어를 지어요',say:'이제 글자를 모아 단어를 지어요. 사라진 말을 되찾아 시든 마을을 살려 볼까요?',
  ...
  8:{act:8,pages:[
    ...
    {cap:'스스로 읽어요',say:'이제 이 편지를 스스로 읽어 볼까요? 그러면 하니가 반짝이는 편지를 온 마을에 배달할 거예요. 준비됐나요?',
