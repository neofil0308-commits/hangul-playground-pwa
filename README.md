# 하니의 한글 모험 (Hani's Hangul Adventure)

> 만 4세, 한글을 거의 모르는 아이가 **그림·소리 중심의 모험**을 따라가며 자음·모음 → 글자 → 단어 → 문장 순으로 한글을 떼는 정적(static) PWA.

📂 구조·내용은 [`ARCHITECTURE.md`](ARCHITECTURE.md), 작업 이력은 [`PROGRESS.md`](PROGRESS.md) 참고.

---

## 1. 개발 목적 (북극성)

**"이 앱의 모험을 끝까지 클리어하면, 아이가 한글을 어느 정도 스스로 읽게 된다."**

- **스토리 진행 순서 = 실제 한글 학습 순서.** 마지막 막을 깨면 아이가 편지를 스스로 읽음(= 한글 뗌).
- 별도의 "공부"가 아니라 **별빛 우체국을 구하는 모험**을 하다 보면 자연스럽게 글자를 익히게 한다.

## 2. 대상 사용자

- **만 4세**, 자음·모음을 거의 모르는 백지 상태.
- **아직 글을 못 읽음** → 글로 된 설명 금지. **큰 그림 + 하니(🐥) 목소리**가 핵심. 글자(자막)는 부모가 같이 읽어주는 보조.
- 사용 환경: **아이패드 가로 모드 또는 모니터(가로 와이드)**. 레이아웃은 가로 와이드를 우선 최적화.

## 3. 설계 원칙 (개발 시 항상 지킬 것)

1. **소리·그림 우선, 글 최소화.** 핵심 정보는 음성·그림으로. 신경망 음성(edge-tts), 기계음·끊김·중복 재생 금지.
2. **주제 일관성.** 한 퀘스트는 '오늘의 글자' 하나 중심. 게임·단어·예시가 모두 그 글자와 연결(무작위 풀 금지).
3. **가르치듯 설명.** 그냥 읽지 말고 "이응에 ㅗ를 더하면 오"처럼 결합 원리를. 단어 동산은 "ㅍ + ㅗ = 포" 구조도.
4. **모양 기억 + 소리 연결.** 4세는 글자를 이미지로 기억하는 단계가 정상. 단, 모양을 보여줄 때 **항상 소리를 함께** 붙여 다중감각으로.
5. **차등 피드백.** 작은 성취(글자 하나)=작게("맞아요!"), 큰 성취(단어·라운드 완성)=크게(중앙 오버레이). 즉시보상 + 큰 마무리보상.
6. **관대한 익힘 판정.** 만나고+짝맞추고+소리퀴즈 정도면 마스터. 좌절 금지.
7. **정답을 그냥 노출하지 않기.** 듣기 활동은 소리로 답을 찾게 한다(정답 글자를 화면에 띄워두지 않음).
8. **글씨와 가이드 구분.** 획순 가이드=분홍, 아이 펜=검정.

## 4. 학습 여정 — 별빛 우체국 8막 (`CURRICULUM`)

| 막 | 주제 | 장소 | 내용 | 상태 |
|----|------|------|------|------|
| 1막 | 모음의 빛 | 글자 숲 | 기본 모음 ㅏㅓㅗㅜㅡㅣㅑㅕㅛㅠ | ✅ 10화 |
| 2막 | 자음 친구들 | 글자 숲 | 기본 자음 ㅁㄴㅇㄱㄷㅂㅅㄹㅈㅊㅋㅌㅍㅎ | ✅ 14화 |
| 3막 | 글자 공방 | 글자 공방 | 자음+모음 합치기(가·나·다) | ✅ 14화 |
| 4막 | 받침의 문 | 받침의 문 | 받침 ㄱㄴㄷㄹㅁㅂㅇ + 붙여보기 | ✅ 14화 |
| 5막 | 쌍둥이 소리 | 소리 동굴 | 된소리 ㄲㄸㅃㅆㅉ | ✅ 5화 |
| 6막 | 숨은 모음 | 글자 숲 | 복모음 ㅐㅔㅒㅖㅚㅟㅢㅘㅝㅙㅞ | ✅ 11화 |
| 7막 | 단어 마을 | 단어 동산 | 난이도순 단어 읽기 | ✅ 12화 |
| 8막 | 이야기 책 | 별빛 우체국 | 문장 읽기 + **피날레**(오프닝의 그 편지) | ✅ 15화 |

- 글자형 막은 **글자 하나하나가 한 에피소드**(`EPISODE_PATH`, 전체 **95화** ≈ 하루 1화로 13주).
- 마지막 화는 **피날레** — 오프닝에서 하얗게 지워진 편지("안녕 하니 / 사랑해 / 또 보자")를 아이가 스스로 읽으면 여정이 닫힌다.
- 빠른 동기부여용 **맛보기(`STORY_MILESTONES`)**: ㅏ+ㅣ를 익히면 곧장 "아이"를 읽었다고 축하.

## 5. 하루(에피소드) 흐름

인트로 그림책(최초 1회) → 홈(상단 여정 status + 오늘의 3모험) → **글자 숲**(소리·낱말·받아쓰기) → **단어 동산**(자모 드래그 조립) → **소리 동굴**(소리 듣고 찾기) → **마무리 정리**(오늘 글자+복습).

## 6. 현재 상태 & 다음 방향

- **됨**: 1~8막 전 막이 모험 플로우에 편입되어 플레이 가능. 글자형 막(1·2·4·5·6막) + **3막 글자 공방(자모 합치기, `openCombine`)** + 7막 단어 동산 + **8막 이야기 책(문장 읽기·졸업, `openStory`)** + 인트로·여정·복습. 가로 와이드 2단 UI·차등 피드백·신경망 음성·획순(음절/쌍자음 합성)까지 정리.
- **정리 완료(2026-07-21)**: 초기 '한글 놀이터' 잔재 화면 5종(`letters/syl/word/sent/sentWrite`)을 제거했다. 도달 경로를 전수 추적해 **전부 진입 불가능한 죽은 화면**임을 확인한 뒤 지웠고, 기능은 이미 `combine`/`story`/`wordBuild`가 대체하고 있었다. (`trace`는 단어 동산 '써보기'로 들어가는 살아있는 화면이라 유지 → `ARCHITECTURE.md` §3)
- **앞으로(무료배포)**: 2026-07-23에 **유료(₩5,000 1회 구매) 모델을 폐기하고 전체 무료로 전환**했다. 폰트 self-host·법적 문서 무료화·잔재 화면 정리·**TWA(Android) 패키징**까지 완료. 남은 것은 Play 스토어 무료 등록뿐이다 — 개발자 계정($25)과 assetlinks 호스팅. 상세는 [`Play_등록_체크리스트.md`](Play_등록_체크리스트.md), 스토어 문안은 [`스토어_등록정보.md`](스토어_등록정보.md). (`유료배포_검토.md`는 유료 시절 이력으로 보존)

## 7. 실행 / 검증

```bash
# 로컬 서버 (같은 WiFi 아이패드 접속 시 --bind 0.0.0.0)
python -m http.server 5399 --bind 0.0.0.0
#  → http://<PC_LAN_IP>:5399/index.html  (가로 모드)

# 콘텐츠 불변식 (예시 낱말·보기 풀·음성 커버리지·막별 분량 …)
python tools/check_content.py

# 스토어 문안 글자 수 (앱 이름 30 / 짧은 설명 80 / 자세한 설명 4000)
python tools/check_store_listing.py

# 정적 회귀 테스트 (223개)
python -m pytest -q tests/ -o python_files="*_check.py"

# 런타임 스모크 (화면 전환·핵심 경로를 실제로 눌러 본다 — 정적 테스트가 못 잡는 회귀용)
npm install jsdom                 # 최초 1회
node tools/smoke_runtime.js       # 위 http.server가 5399 포트로 떠 있어야 함

# 화면 캡처 (진짜 Chrome — jsdom이 못 보는 '보이는 문제'용. shots/ 에 PNG)
npm install puppeteer-core        # 최초 1회. 브라우저는 시스템 Chrome을 쓴다
node tools/screenshot.js
```

> 편집 후 옛 화면이 보이면: 브라우저에서 서비스워커/캐시 비우고 두 번 새로고침(자세히 `ARCHITECTURE.md` §7). 실기기 접속 불가면 방화벽(5399 인바운드) 확인.
