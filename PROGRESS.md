...
> 마지막 업데이트: 2026-07-22 (브라우저 실렌더 검증 + chrome-devtools MCP 등록. 다음 세션은 위 "다음 작업(우선순위)" 참고)
>
...
> 유료배포 로드맵은 3단계(법적 문서 초안)까지 완료, 4단계(패키징·결제)가 남음.
...
### B. 유료배포 로드맵 4단계 (사용자 액션 선행 필요) — 상세 `유료배포_검토.md`
- 🔴 **사업자등록 + 통신판매업 신고** → 법적 문서 자리표시자 **14건** 기입(`python tools/check_legal_placeholders.py`로 확인, 0이어야 판매 가능).
- 🔴 **Google Play 개발자 계정** $25(1회, Android 먼저 권장).
- 🟠 그 다음 내가 할 수 있는 것: Bubblewrap으로 TWA 생성 → Play Billing 비소모성 상품 연동 → `unlockPremium()` 연결 + 구매 복원. **`UNLOCK_CODE='HANI-2026'` 평문 상수 폐기**(정적 배포라 소스 공개됨 → IAP로 대체).
